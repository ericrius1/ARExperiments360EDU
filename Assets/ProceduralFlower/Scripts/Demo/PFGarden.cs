...
namespace mattatz.ProceduralFlower.Demo {
	...
	public class PFGarden : MonoBehaviour {
        ...
        void Start()
        {
            ...
            InvokeRepeating("GrowFlower", 5, 3);
        }
        ...
        void GrowFlower()
        {
            ...
            GameObject flower = Instantiate(prefabs[Random.Range(0, prefabs.Count)]) as GameObject;
            flower.transform.position = position;
            flower.transform.localScale = Vector3.one * Random.Range(0.4f, 0.5f);
            flower.transform.localRotation = Quaternion.LookRotation(Vector3.forward, normal);

            Debug.Log("Grow!!!");
            numFlowers++;
            if(numFlowers >= MaxFlowers )
            {
                CancelInvoke("GrowFlower");
            }
        }
